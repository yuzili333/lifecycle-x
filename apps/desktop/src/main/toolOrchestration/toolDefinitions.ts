...
function toolDescription(toolKind: ToolKind) {
  if (toolKind === "sql_query") {
    return "request_sql_query_execution 是受控只读 SQL 查询执行请求工具。SQL 必须经过安全校验、权限校验和用户审批，不得直接伪造查询结果。";
  }
  if (toolKind === "python_analysis") {
    return "request_python_analysis_execution 是受控 Python 数据分析工具。默认使用会话最新成功 SQL 结果作为输入，必须经过审批和沙箱执行。";
  }
  ...
  return "request_markdown_report_generation 是受控 Markdown 报告生成工具。报告必须引用 Artifact 和工具结果摘要，不得编造数据，每次修改生成新版本。";
}
