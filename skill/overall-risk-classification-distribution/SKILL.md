...
- `analysisScope.dataSourceName`/`{{dataSourceName}}` 必须填写用户当前真实选择的数据源表名称，优先使用客户端选中数据源的 `name`、CSV 导入表名或数据库表名；不得填写“用户选择数据源”等固定占位文本。数据库、环境或连接类型可放入数据血缘说明，不得替代表名称。
- `provenance.generatedAt`/`{{generatedAt}}` 必须渲染为 `YYYY-MM-DD HH:mm:ss`，不得输出 ISO 时间戳。
...
11. 报告中的数据来源 `dataSourceName` 必须取用户当前选择的数据源表名称，不得写成“用户选择数据源”等占位文本；报告生成时间必须为 `YYYY-MM-DD HH:mm:ss`。
12. 报告分析结论必须包含 `【笔数维度】`、`【金额维度】`、`【正常类维度】` 三个小节；其中 `【正常类维度】` 必须说明十二级分类 `latest_risk_result` 中含“正常1”“正常2”“正常3”的数据总计笔数，并说明正常1、正常2、正常3内部结构。
13. 如果最新风险分类结果 `latest_risk_result` 字段不存在，应明确报告限制并跳过十二级分类分析，不得使用 `latest_risk` 代替。
14. 不得编造缺失数据，不得根据 preview rows 推断全量结论。
